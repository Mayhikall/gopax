...
abstract contract PaymasterERC20GuarantorMock is PaymasterERC20Mock, PaymasterERC20Guarantor {
    ...
    bytes32 private constant PACKED_USER_OPERATION_TYPEHASH = keccak256(
        "PackedUserOperation(address sender,uint256 nonce,bytes initCode,bytes callData,bytes32 accountGasLimits,uint256 preVerificationGas,bytes32 gasFees,bytes paymasterAndData)"
    );

    function _fetchGuarantor(PackedUserOperation calldata userOp)
        internal
        view
        virtual
        override
        returns (address guarantor)
    {
        bytes calldata paymasterData = userOp.paymasterData();
    ...
    function _postOpGasBudget(PackedUserOperation calldata userOp)
        internal
        view
        virtual
    ...
    function _refund(
        ...
        return
            super._refund(
                token, actualAmount, tokenPrice, actualUserOpFeePerGas, prefunder, prefundAmount, prefundContext
            );
    }
    ...
    function _getStructHashWithoutOracleAndGuarantorSignature(PackedUserOperation calldata userOp)
        private
        pure
        returns (bytes32)
    {
...
abstract contract PaymasterERC20ReducingMock is PaymasterERC20 {
    function _prefund(
        ...
        return super._prefund(
            userOp, userOpHash, token, tokenPrice, prefunder_, prefundAmount_ == 0 ? 0 : prefundAmount_ - 1
        );
    }
...
contract PaymasterERC20GuarantorReducingMock is PaymasterERC20ReducingMock, PaymasterERC20Guarantor {
    function _fetchDetails(PackedUserOperation calldata, bytes32)
        internal
        view
        virtual
        override
    ...
    function _refund(
        ...
        return super._refund(
            token, tokenPrice, actualAmount_, actualUserOpFeePerGas, prefunder, prefundAmount, prefundContext
        );
    }
}
