...
abstract contract PaymasterSigner is AbstractSigner, EIP712, Paymaster {
    ...
    bytes32 private constant USER_OPERATION_REQUEST_TYPEHASH = keccak256(
     ...
     */
    function _signableUserOpHash(PackedUserOperation calldata userOp, uint48 validAfter, uint48 validUntil)
        internal
        view
        virtual
        returns (bytes32)
    ...
    function _decodePaymasterUserOp(PackedUserOperation calldata userOp)
        internal
        pure
        virtual
        returns (uint48 validAfter, uint48 validUntil, bytes calldata signature)
    {
        bytes calldata paymasterData = userOp.paymasterData();
        return paymasterData.length < 12
